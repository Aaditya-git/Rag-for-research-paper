...
def chunk_text_semantic(text, max_chunk_size=1000, similarity_threshold=0.7, source_metadata=None):
    """
    Semantic chunking: Groups sentences by semantic similarity.
    Uses embeddings to determine where natural topic boundaries are.
    """
    if not text or not text.strip():
        return []
    ...
    print("Computing semantic boundaries...")
    chunks = []
    ...
    for i in range(1, len(sentences)):
        # Calculate similarity between current sentence and chunk so far
        current_chunk_text = " ".join(current_chunk_sentences)
        test_chunk_text = current_chunk_text + " " + sentences[i]
        
        # Check if adding this sentence would exceed max size
        if len(test_chunk_text) > max_chunk_size:
            # Save current chunk
            page_num = None
            for marker_pos, page in page_markers:
                if marker_pos <= char_position:
            ...
            chunk_data = create_text_chunk_metadata(
                ...
                chunk_id=len(chunks),
                char_start=char_position,
                char_end=char_position + len(current_chunk_text),
                source_metadata=source_metadata,
                page_number=page_num,
            ...
            char_position += len(current_chunk_text)
            current_chunk_sentences = [sentences[i]]
            current_chunk_start = i
        else:
            # Calculate semantic similarity
            chunk_embedding = np.mean(sentence_embeddings[current_chunk_start:i], axis=0).reshape(1, -1)
...
def chunk_markdown_file(md_file, chunk_size=1000, overlap=200, use_semantic=False, similarity_threshold=0.7):
    """
    Chunk a markdown file using either recursive or semantic chunking.
    
    Args:
        md_file: Path to markdown file
        chunk_size: Maximum chunk size
        overlap: Overlap for recursive chunking
        use_semantic: If True, use semantic chunking; otherwise use recursive
        similarity_threshold: Threshold for semantic similarity (0-1)
    """
    md_path = Path(md_file)
    
    print(f"\nChunking: {md_path.name}")
    print(f"Method: {'Semantic' if use_semantic else 'Recursive'}")
    ...
    if use_semantic:
        chunks = chunk_text_semantic(text, chunk_size, similarity_threshold, source_metadata)
    ...
    output_file = output_dir / f"{base_name}_chunks_{method_name.lower()}.md"
